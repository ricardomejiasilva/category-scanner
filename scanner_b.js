...
const CONFIG = {
  ...
  sites: [
    ...
    // Template C: a.fancy-product-card
...
const EXCLUDED_PATHS = [
  ...
  '/page/', '/support', '/policies', '/equipment-parts',
  '.xml', '.pdf', '/shop-by-collection/',
];
...
async function main() {
  ...
  const sites = singleSite
    ? [`https://${singleSite.replace(/^https?:\/\//, '')}`]
    : CONFIG.sites; // each entry may be a string or { url, selector }
